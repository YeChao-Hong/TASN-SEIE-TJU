    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(crops[0])):
            ...
            if faces:
                ...
                top = int(0.5 * h)  # 相对于人脸检测框的下半部起点
                # 注意这里 mouth_region 还是基于 face_np，所以要加上 face 检测框的偏移 x,y
                mx1 = x + ml[0] - int((mr[0] - ml[0]) * 0.5)
                my1 = y + top
                mw = (mr[0] - ml[0])
                mh = (h - top)
                mside = max(mw, mh)
                ...
                # 回退到原始分块
                # crops[1].append(transforms.Resize((224, 224))(face_tensor))
                # crops[2].append(transforms.Resize((224, 224))(face_tensor))
